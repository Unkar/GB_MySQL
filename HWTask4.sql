DROP DATABASE IF EXISTS hwtask4;
CREATE DATABASE hwtask4;
USE hwtask4;
...
SELECT `murzik_the_cat`.`name`, `shops`.`shopname`
FROM `shops`
JOIN (SELECT `name`, `shops_id`
		FROM `cats`
        WHERE `name` = 'Murzik') AS `murzik_the_cat`
ON shops.id = `murzik_the_cat`.shops_id;
...
JOIN Orders o
ON a.an_id = o.ord_an
WHERE (o.ord_datetime >= '2020-02-05 00:00:00' AND o.ord_datetime < '2020-02-12 00:00:00');

-- если имелось ввиду вывести типы анализов
